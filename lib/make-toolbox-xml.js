...
/* eslint-disable no-unused-vars */
const ipp = function (isInitialSetup, isStage, targetId) {
    const stageSelected = ScratchBlocks.ScratchMsgs.translate(
        'MOTION_STAGE_SELECTED',
        'Stage selected: no motion blocks'
    );
    return `
    <category name="%{BKY_CATEGORY_IPP}" id="ipp" colour="#4C97FF" secondaryColour="#3373CC">
        ${isStage ? `
        <label text="${stageSelected}"></label>
        ` : `
        <block type="car_to">
            <value name="TO">
                <shadow type="car_to_menu">
                </shadow>
            </value>
            <value name="SPEED">
                <shadow type="math_number">
...
const makeToolboxXML = function (isInitialSetup, device = null, isStage = true, targetId, categoriesXML = [],
    ...
    if (device) {
        ...
        const ippXML = moveCategory('ipp') || ipp(isInitialSetup, isStage, targetId);
